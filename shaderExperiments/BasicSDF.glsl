...
float scene(vec3 currentRayPosition) {    
    ...
    float cube = sdfBox(pos, vec3(0.9));
    
    float sphere = sdfSphere(pos, 1.2);
    
    vec3 ballOffset = vec3(0.4 + 1.0 + sin(1.7 * time), 0.0, 0.0);
    float ballRadius = 0.3;
    float balls = sdfSphere(pos - ballOffset, ballRadius);
    balls = sdfUnion(balls, sdfSphere(pos + ballOffset, ballRadius));
    balls = sdfUnion(balls, sdfSphere(pos - ballOffset.yxz, ballRadius));
    balls = sdfUnion(balls, sdfSphere(pos + ballOffset.yxz, ballRadius));
    balls = sdfUnion(balls, sdfSphere(pos - ballOffset.yzx, ballRadius));
    balls = sdfUnion(balls, sdfSphere(pos + ballOffset.yzx, ballRadius));
